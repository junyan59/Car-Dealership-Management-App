<?php

include('lib/common.php');

if (!isset($_SESSION['userName'])) {
	header('Location: login.php');
	exit();
}

$query = "SELECT user_type " .
		 "FROM User " .
		 "WHERE User.userName = '{$_SESSION['userName']}'";

$result = mysqli_query($db, $query);

if (!is_bool($result) && (mysqli_num_rows($result) > 0) ) {
        $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
		$user_type = $row['user_type'];
} else {
        array_push($error_msg,  "Sorry, the user account does not exist!" );
}

if ($_SERVER['REQUEST_METHOD'] == 'POST') {

	$taxID = mysqli_real_escape_string($db, $_POST['taxID']);
	$business_name = mysqli_real_escape_string($db, $_POST['business_name']);
	$contact_name = mysqli_real_escape_string($db, $_POST['contact_name']);
	$contact_title = mysqli_real_escape_string($db, $_POST['contact_title']);
	$Baddress = mysqli_real_escape_string($db, $_POST['Baddress']);
	$Bphone_number = mysqli_real_escape_string($db, $_POST['Bphone_number']);
	$Bemail = mysqli_real_escape_string($db, $_POST['Bemail']);
	$customer_ID = mysqli_real_escape_string($db, $_POST['customer_ID']);
	

	if (!empty($taxID) && !empty($business_name) && !empty($contact_name) && !empty($contact_title) && !empty($Baddress) && !empty($Bphone_number) && !empty($customer_ID) ) {
		$query = "INSERT INTO Customer (customer_ID) VALUES ('$customer_ID')";

		$result = mysqli_query($db, $query);
        include('lib/show_queries.php');

        if (mysqli_affected_rows($db) == -1) {
        	array_push($error_msg,  "UPDATE ERROR: Customer ID... <br>".  __FILE__ ." line:". __LINE__ );
        }


	}

	if (!empty($taxID) && !empty($business_name) && !empty($contact_name) && !empty($contact_title) && !empty($Baddress) && !empty($Bphone_number) && !empty($customer_ID) ) {

		$query = "INSERT INTO Business (taxID, business_name, contact_name, contact_title, Baddress, Bphone_number, Bemail, customer_ID) VALUES ('$taxID', '$business_name', '$contact_name', '$contact_title', '$Baddress', '$Bphone_number', '$Bemail', '$customer_ID')";
	
		include('lib/show_queries.php');
		$result = mysqli_query($db, $query);

		if (mysqli_affected_rows($db) == -1) {
        	array_push($error_msg,  "UPDATE ERROR: Business Customer... <br>".  __FILE__ ." line:". __LINE__ );
        }

        if (mysqli_affected_rows($db) > 0) {
        	array_push($error_msg, "Business Customer Successfully Added!");
        }

        }
	
}	
?>




<?php include("lib/header.php"); ?>
	<body>
    	<div id="main_container">
		<?php
			if ($user_type == "Manager, Sales Person, Clerk"){
				include("lib/menu_owner.php");
			} elseif ($user_type == "Manager"){
				include("lib/menu_manager.php");
			} elseif ($user_type == "Sales Person"){
				include("lib/menu_salespeople.php");
			} elseif ($user_type == "Clerk"){
				include("lib/menu_inventory_clerks.php");
			}
		?>
			<div class="center_content">
				<div class="center_left">
					<div class="features">
						<div class="profile_section">
                        	<div class="subtitle">Add Business Customer Form</div>
								<form name="addBusinessForm" action="add_business_customer.php" method = "POST">
									<table>
										<tr>
											<td class="item_lable">Tax ID</td>
											<td><input type="text" name="taxID" /></td>
										</tr>
										<tr>
											<td class="item_lable">Business Name</td>
											<td><input type="text" name="business_name" /></td>
										</tr>
										<tr>
											<td class="item_lable">Contact Name</td>
											<td><input type="text" name="contact_name" /></td>
										</tr>
										<tr>
											<td class="item_lable">Contact Title</td>
											<td><input type="text" name="contact_title" /></td>
										</tr>
										<tr>
											<td class="item_lable">Business Address</td>
											<td><input type="text" name="Baddress" /></td>
										</tr>					
										<tr>
											<td class="item_label">Business Phone Number</td>
											<td><input type="text" name="Bphone_number" /></td>
										</tr>
										<tr>
											<td class="item_label">Business Email</td>
											<td><input type="text" name="Bemail" /></td>
										</tr>
										<tr>
											<td class="item_label">Customer ID</td>
											<td><input type="text" name="customer_ID" /></td>
										</tr>
									
									<td><a href="javascript:addBusinessForm.submit();" class="fancy_button">Add Business Customer</a></td>
								</table>
								</form>
							</div>
						 </div>
					</div>

                <?php include("lib/error.php"); ?>

				<div class="clear"></div>
			</div>

		</div>
	</body>
</html>